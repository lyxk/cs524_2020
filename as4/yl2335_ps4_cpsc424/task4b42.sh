...
# In this example, we've asked Slurm for 8 tasks, but we've asked mpirun for 4 procs, which will go onto 2 nodes.
echo ""
echo "p = 4"
time mpirun -n 4 --map-by socket --rank-by socket ./task4 -b
time mpirun -n 4 --map-by socket --rank-by socket ./task4 -b
time mpirun -n 4 --map-by socket --rank-by socket ./task4 -b
