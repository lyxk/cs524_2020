...
// Assume the matrix dimensions are multiples of the tile width
__global__ void gpu_matrixmult(FP *a, FP *b, FP *c, int n, int p, int m, int TW, int NTB) {
  ...
  // if (bx * NTB * TW >= m) return;

  int row = ty + by * blockDim.y, col = tx + bx * NTB * TW;
  int indexa, indexb;

  // Loop over tiles
  for (int t = 0; t < p / TW; t++) {
    // Reset col
    col = tx + bx * NTB * TW;

    indexa = row * p + t * TW + tx;
    atile[ty * TW + tx] = a[indexa]; // Copy to shared memory
    ...
    for (int t1 = 0; t1 < NTB; t1++, col += TW) {
      ...
      btile[ty * TW + tx] = b[indexb]; // Copy to shared memory
      __syncthreads();
...
int main(int argc, char *argv[]) {
  ...
  printf("Block_Dim = (%d, %d), Grid_Dim = (%d, %d), NTB = %d, \n", Block_Dim_x, Block_Dim_y, Grid_Dim_x, Grid_Dim_y, NTB);
